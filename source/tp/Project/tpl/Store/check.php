...
<div class="mainnav_title" id="nav">
	<a onclick="javascript:slidowns(1)" <if condition="$is_check neq '2'">	class="on"</if> href="{pigcms{:U('Store/check', array('id' => $store_id))}">未对账订单列表</a>|
	...
	<table class="ui-table ui-table-list" style="padding: 0px;">
		...
		<tr class="widget-list-header">
			...
			<th class="cell-10 text-right"><if condition="$is_check eq '2'">已</if>提成比例</th>
		...
		<tr class="widget-list-item">
			...
			<td class="text-right ui-money ui-money-outlay"><if condition="$is_check eq '2'">{pigcms{$order['sales_ratio']}<else/>{pigcms{$info['sys_sales_ratio']}</if></td>
			...
			<td class="zt">
				<if condition="$is_check eq '1'">
					<input class="button qrcz" type="button" value="对账" store_id="{pigcms{$order['store_id']}" data-order-id="{pigcms{$order['order_id']}"   data-order-no="{pigcms{$order['order_no']}" ><br>
				<else/>
					已对账
				</if>
			</td>
	...
	<if condition="$orders neq ''">
		...
		<table class="ui-table">
				...
				<table class="ui-table">
					<tr>
						<td class="pagebar" colspan="6">按系统现：<font class="ui-money-outlay">{pigcms{$info['sys_sales_ratio']}</font> 提成标准：本页现对账情况</td>
					</tr>
					<tr>
						<td>本页合计已对账金额：</td>
						<td>￥{pigcms{$info['page_uncheck_account']}元</td>
