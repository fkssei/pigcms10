			...
			$(function(){
				...
				$('.js-confirm-choose').live('click',function(){
					...
					$.each($('.js-choose.btn-primary'),function(i,item){
						...
						data_arr[i] = {'id':$(item).data('id'),'title':$(item).data('title'),'face_money':$(item).data('facemoney'),'condition':$(item).data('condition')};
					});
					//alert("555555555555");
					//alert(data_arr);
							...
							<?php foreach($coupons as $product){ ?>
								<tr>
									<td class="title" style="max-width:300px;">
										<div class="td-cont">
											<a target="_blank" class="new_window" href="<?php echo $config['wap_site_url'];?>/good.php?id=<?php echo $product['product_id'];?>"><?php echo $product['face_money']; ?></a>
										</div>
									</td>
									<td class="time">
										<div class="td-cont">
