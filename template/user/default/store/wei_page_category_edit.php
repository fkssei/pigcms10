<div class="app-design clearfix">
    <div class="app-preview">
        <div class="app-header"></div>
        <div class="app-entry">
            <div class="app-config js-config-region">
                <div class="app-field clearfix editing">
					<h1><span></span></h1>
					<div class="control-group">
						<div class="custom-title text-left">
							<h2 class="title"></h2>
						</div>
					</div>
					<div class="control-group">
						<div class="custom-richtext"></div>
					</div>
					<div class="control-group">
						<ul class="custom-nav custom-nav-noicon clearfix">
							<li>
								<a class="clearfix" href="javascript:;" target="_blank">
									<span class="custom-nav-title">第0条微页面标题</span>
									<i class="right right-arrow"></i>
								</a>
							</li>							
							<li>
								<a class="clearfix" href="javascript:;" target="_blank">
									<span class="custom-nav-title">第1条微页面标题</span>
									<i class="right right-arrow"></i>
								</a>
							</li>							
							<li>
								<a class="clearfix" href="javascript:;" target="_blank">
									<span class="custom-nav-title">第N条微页面标题</span>
									<i class="right right-arrow"></i>
								</a>
							</li>						
						</ul>
					</div>
					<div class="actions">
						<div class="actions-wrap"><span class="action edit">编辑</span><span class="action add">加内容</span></div>
					</div>
				</div>
            </div>
			<div class="app-fields js-fields-region"><div class="app-fields ui-sortable"></div></div>
		</div>
    </div>
    <div class="app-sidebars">
		<div class="app-sidebar" style="margin-top:71px;">
			<div class="arrow"></div>
			<div class="app-sidebar-inner js-sidebar-region"></div>
		</div>
    </div>
    <div class="app-actions" style="display:block;bottom:0px;">
        <div class="form-actions text-center">
            <input class="btn btn-primary btn-save" type="submit" value="保存" data-loading-text="保存..."/>
            <!--input class="btn btn-preview" type="submit" value="预览效果" data-loading-text="预览效果..."/-->
        </div>
    </div>
</div>
<div style="display:none;" id="edit_data" cat-name="<?php echo $now_category['cat_name'];?>" cat-src="<?php echo $now_category['cover_img'];?>" cat-id="<?php echo $now_category['cat_id'];?>" first-sort="<?php echo $now_category['first_sort'];?>" second-sort="<?php echo $now_category['second_sort'];?>"><?php echo $now_category['cat_desc'];?></div>
<div style="display:none;" id="edit_custom" custom-field='<?php echo $customField;?>'></div>